...
(async () => {
    ...
    try {
        ...
        for (let pointerToProject = 0; pointerToProject < chunkOfProjectDocument.length; pointerToProject++) {
            ...
            // get project documents from projects collection in Array
                    ...
                    if(solutionDocument.length == 1){
                        ...
                        if(projectDocument[counter].hasOwnProperty("userProfile"))
                        {
                            ...
                            for(let i = 0; i < userLocations.length; i++){
                                if(userLocations[i].type !== "school"){
                                    userRoleInfomration[userLocations[i].type] = userLocations[i].id
                                }else{
                                    userRoleInfomration[userLocations[i].type] = userLocations[i].code
                                }
                            }
                            userRoleInfomration.Role = projectDocuments[counter].userProfile.profileUserType.subType ? projectDocuments[counter].userProfile.profileUserType.subType.toUpperCase() : projectDocuments[counter].userProfile.profileUserType.type.toUpperCase()
