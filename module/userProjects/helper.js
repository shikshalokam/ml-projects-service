...
const projectTemplatesHelper = require(MODULES_BASE_PATH +
  "/project/templates/helper");
const projectTemplateTasksHelper = require(MODULES_BASE_PATH +
  "/project/templateTasks/helper");
...
const certificateService = require(GENERICS_FILES_PATH +
...
module.exports = class UserProjectsHelper {
  ...
  static sync(
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (
          userProject[0].lastDownloadedAt.toISOString() !== lastDownloadedAt
        ) {
          throw {
            status: HTTP_STATUS_CODE["bad_request"].status,
        ...
        if (userProject[0].status == CONSTANTS.common.SUBMITTED_STATUS) {
          throw {
            status: HTTP_STATUS_CODE["bad_request"].status,
            message:
              CONSTANTS.apiResponses.FAILED_TO_SYNC_PROJECT_ALREADY_SUBMITTED,
          };
        }

        const projectsModel = Object.keys(schemas["projects"].schema);

        let keysToRemoveFromUpdation = [
          "userRoleInformation",
          "userProfile",
          "certificate",
        ];
        ...
        if (createNewProgramAndSolution || solutionExists) {
          let programAndSolutionInformation =
            await this.createProgramAndSolution(
              ...
              userToken,
              userProject[0].solutionInformation &&
          ...
          if (solutionExists) {
            let updateProgram = await surveyService.removeSolutionsFromProgram(
              userToken,
              userProject[0].programInformation._id,
              [userProject[0].solutionInformation._id]
            );
        ...
        Object.keys(data).forEach((updateData) => {
          if (
            ...
            if (booleanData.includes(updateData)) {
              updateProject[updateData] = UTILS.convertStringToBoolean(
                data[updateData]
              );
  ...
  static createProgramAndSolution(
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        result.solutionInformation._id = ObjectId(
        ...
        result["isAPrivateProgram"] =
          result.programInformation.isAPrivateProgram;
        ...
        if (solutionAndProgramCreation.data.parentSolutionInformation) {
          result["link"] = solutionAndProgramCreation.data
            .parentSolutionInformation.link
  ...
  static getProgramAndSolutionDetails(solutionDetails) {
    return new Promise(async (resolve, reject) => {
      try {
        let result = {};
        result.solutionInformation = _.pick(solutionDetails, [
          "name",
          "externalId",
          "description",
          "_id",
        ...
        result["link"] =
          solutionData.length > 0 && solutionData[0].link
            ? solutionData[0].link
            : "";
      ...
      try {
        ...
        for (let task = 0; task < projectTasks.length; task++) {
          ...
          if (
            ...
            let minNoOfSubmissionsRequired = currentTask.solutionDetails
            ...
            if (currentTask.submissions && currentTask.submissions.length > 0) {
              ...
              completedSubmissionCount = currentTask.submissions.filter(
                (eachSubmission) =>
                  eachSubmission.status === CONSTANTS.common.COMPLETED_STATUS
              ).length;
              ...
              if (completedSubmissionCount >= minNoOfSubmissionsRequired) {
                ...
                completedSubmissionDoc = currentTask.submissions.find(
                  (eachSubmission) =>
                    eachSubmission.status === CONSTANTS.common.STARTED
                );
              }
  ...
  static pushSubmissionToTask(projectId, taskId, updatedData) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let currentTask = projectDocument[0].tasks.find(
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (
          ...
          assessmentOrObservationData["entityType"] =
            project[0].entityInformation.entityType;
        }

        if (
          currentTask.solutionDetails &&
          !_.isEmpty(currentTask.solutionDetails)
        ) {
          assessmentOrObservationData.solutionDetails =
            currentTask.solutionDetails;
        }
  ...
  static detailsV2(
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (templateId !== "") {
          let templateDocuments = await projectTemplateQueries.templateDocument(
            {
        ...
        if (projectId === "") {
          ...
          const targetedSolutionId =
            await coreService.checkIfSolutionIsTargetedForUserProfile(
              userToken,
              userRoleInformation,
              solutionId
            );
          ...
          const projectDetails = await projectQueries.projectDocument(
            {
              solutionId: solutionId,
              userId: userId,
              isAPrivateProgram: targetedSolutionId.data.isATargetedSolution
                ? false
                : true,
            },
            ["_id"]
          ...
          if (projectDetails.length > 0) {
            ...
            let isAPrivateSolution =
              targetedSolutionId.data.isATargetedSolution === false
                ? true
                : false;
            let solutionDetails = {};
            ...
            if (templateId === "") {
              ...
              if (isAPrivateSolution && solutionId != "") {
                ...
                if (
                  ...
                  throw {
                    status: HTTP_STATUS_CODE["bad_request"].status,
                    message:
                      CONSTANTS.apiResponses.SOLUTION_DOES_NOT_EXISTS_IN_SCOPE,
                  };
                }
              ...
              solutionDetails = await surveyService.listSolutions([
                solutionExternalId,
              ]);
            ...
            let programDetails = await programsQueries.programsDocument(
              queryData,
            ...
            if (
              ...
              if (certificateTemplateDetails.length > 0) {
                projectCreation.data["certificate"] = _.pick(
                  certificateTemplateDetails[0],
                  ["templateUrl", "status", "criteria"]
                );
            ...
            if (bodyData && Object.keys(bodyData).length > 0) {
              if (bodyData.hasAcceptedTAndC) {
                projectCreation.data.hasAcceptedTAndC =
                  bodyData.hasAcceptedTAndC;
              }
              if (bodyData.link) {
              ...
              if (
                solutionDetails.entityType &&
                bodyData[solutionDetails.entityType]
              ) {
                let entityInformation =
                  await surveyService.listEntitiesByLocationIds(userToken, [
                    bodyData[solutionDetails.entityType],
                  ]);
            ...
            if (getUserProfileFromObservation) {
              ...
              if (
                ...
                Object.keys(observationDetails.data.userRoleInformation)
                  .length > 0
              ) {
                userRoleInformation =
                  observationDetails.data.userRoleInformation;
              ...
              let userProfileData = await userProfileService.profile(
                userToken,
                userId
              );
              if (
            ...
            if (
              ...
              let updatedUserProfile =
                await _updateUserProfileBasedOnUserRoleInfo(
                  projectCreation.data.userProfile,
                  userRoleInformation
                );
            ...
            let project = await projectQueries.createProject(
        ...
        if (revertStatusorNot) {
          projectDetails.data.status = UTILS.revertProjectStatus(
            projectDetails.data.status
          );
        } else {
          projectDetails.data.status = UTILS.convertProjectStatus(
            projectDetails.data.status
          );
        }
        ...
        if (
          ...
          if (certificateTemplateDownloadableUrl.success) {
            projectDetails.data.certificate.templateUrl =
              certificateTemplateDownloadableUrl.data[0].url;
          }
        }
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        createProject["userId"] =
          createProject["createdBy"] =
          createProject["updatedBy"] =
            userId;
        ...
        if (data.programId && data.programId !== "") {
          ...
          let programDetails = await programsQueries.programsDocument(
            queryData,
            ["_id", "name", "description", "isAPrivateProgram"]
          );
        ...
        Object.keys(data).forEach((updateData) => {
          if (
            ...
            if (booleanData.includes(updateData)) {
              createProject[updateData] = UTILS.convertStringToBoolean(
                data[updateData]
              );
            } else if (mongooseIdData.includes(updateData)) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        projectDocument.programName = projectDocument.programInformation
          ...
          : "";
        projectDocument.remarks = projectDocument.remarks
          ? projectDocument.remarks
          : "";
        ...
        let evidenceLink = [];
        if (
          projectDocument.attachments &&
          projectDocument.attachments.length > 0
        ) {
        ...
        if (
          projectDocument.categories &&
          projectDocument.categories.length > 0
        ) {
        ...
        if (
          ...
          projectDocument.recommendedFor.length > 0
        ) {
        ...
        if (UTILS.revertStatusorNot(appVersion)) {
          projectDocument.status = UTILS.revertProjectStatus(
            projectDocument.status
          );
        }
        let response = await reportService.projectAndTaskReport(
          userToken,
          projectDocument,
          projectPdf
        );
            ...
            if (templateFilePath.length > 0) {
              ...
              data.forEach((projectData) => {
                if (projectData.certificate) {
                  var itemFromUrlArray =
                    certificateTemplateDownloadableUrl.data.find(
                      (item) =>
                        item.filePath == projectData.certificate.templateUrl
                    );
                  if (itemFromUrlArray) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (requestedData.entityId && requestedData.entityId !== "") {
          let entityInformation = await _entitiesInformation([
        ...
        if (
          ...
          if (
            ...
            solutionDetails[0].parentSolutionId
          ) {
            ...
            programAndSolutionInformation =
              await this.getProgramAndSolutionDetails(solutionDetails[0]);
          } else {
            programAndSolutionInformation = await this.createProgramAndSolution(
        ...
        if (
          ...
          // create certificate object and add data if certificate template is present.
          if (certificateTemplateDetails.length > 0) {
            libraryProjects.data["certificate"] = _.pick(
              certificateTemplateDetails[0],
              ["templateUrl", "status", "criteria"]
            );
          }
          libraryProjects.data["certificate"]["templateId"] =
            libraryProjects.data.certificateTemplateId;
        ...
        libraryProjects.data.projectTemplateExternalId =
          libraryProjects.data.externalId;
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let validateCriteria =
          await certificateValidationsHelper.criteriaValidation(data);
        if (validateCriteria.success) {
          eligible = true;
        } else {
          updateObject["$set"]["certificate.message"] =
            validateCriteria.message;
        }
  ...
  static createCertificate(certificateData, projectId) {
    return new Promise(async (resolve, reject) => {
      try {
        const certificateDetails = await certificateService.createCertificate(
          certificateData
        );
        ...
        if (
          ...
          if (first2 === "1-") {
            ...
            updateObject["$set"]["certificate.transactionId"] =
              transactionIdvalue[1];
          } else {
            updateObject["$set"]["certificate.transactionId"] =
              transactionIdvalue;
          }
        ...
        if (
          ...
          updateObject["$set"]["certificate.osid"] =
            certificateDetails.data.ProjectCertificate.osid;
  ...
  static certificateReIssue(projectId) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let userProfileData = await userProfileService.profileReadPrivate(
          userProject[0].userId
        );
        if (
          userProfileData.success &&
          userProfileData.data &&
          ...
          userProject[0].userProfile.firstName =
            userProfileData.data.response.firstName;
          userProject[0].userProfile.lastName =
        ...
        if (userProject[0].certificate.transactionId) {
          updateObject["$set"][
            "certificate.originalTransactionInformation.transactionId"
          ] = userProject[0].certificate.transactionId;
        }
        if (userProject[0].certificate.osid) {
          updateObject["$set"][
            "certificate.originalTransactionInformation.osid"
          ] = userProject[0].certificate.osid;
        }

        if (userProject[0].userProfile.firstName) {
          updateObject["$set"]["userProfile.firstName"] =
            userProject[0].userProfile.firstName;
        }

        if (userProject[0].userProfile.lastName) {
          updateObject["$set"]["userProfile.lastName"] =
            userProject[0].userProfile.lastName;
        }
    ...
    try {
      ...
      if (project.attachments && project.attachments.length > 0) {
        ...
        for (
          ...
          let currentProjectAttachment =
            project.attachments[pointerToAttachment];
          if (
            currentProjectAttachment.type ==
            CONSTANTS.common.ATTACHMENT_TYPE_LINK
          ) {
        ...
        if (
          projectAttachmentsUrl.data &&
          projectAttachmentsUrl.data.length > 0
        ) {
      ...
      if (project.tasks && project.tasks.length > 0) {
        ...
        if (project.taskSequence && project.taskSequence.length > 0) {
          project.tasks = taskArrayBySequence(
            project.tasks,
...
function taskArrayBySequence(taskArray, sequenceArray, key) {
  var map = sequenceArray.reduce(
    (acc, value, index) => ((acc[value] = index + 1), acc),
    {}
  );
...
function _attachmentInformation(
  ...
  return new Promise(async (resolve, reject) => {
    try {
          ...
          if (type === CONSTANTS.common.PROJECT_ATTACHMENT) {
            attachmentsUrl.data.forEach((eachAttachment) => {
              let projectAttachmentIndex = attachments.findIndex(
                (attachmentData) =>
                  attachmentData.sourcePath == eachAttachment.filePath
              );
            ...
            attachmentsUrl.data.forEach((taskAttachments) => {
              let taskIndex = tasks.findIndex(
                (task) =>
                  task._id ===
                  mapTaskIdToAttachment[taskAttachments.filePath].taskId
              );

              if (taskIndex > -1) {
                let attachmentIndex = tasks[taskIndex].attachments.findIndex(
                  (attachment) =>
                    attachment.sourcePath === taskAttachments.filePath
                );

                if (attachmentIndex > -1) {
                  tasks[taskIndex].attachments[attachmentIndex].url =
                    taskAttachments.url;
                }
              }
            });
      ...
      if (linkAttachments && linkAttachments.length > 0) {
        if (type === CONSTANTS.common.PROJECT_ATTACHMENT) {
          ...
          Object.keys(linkAttachments).forEach((eachTaskId) => {
            let taskIdIndex = tasks.findIndex(
              (task) => task._id === eachTaskId
            );
            if (taskIdIndex > -1) {
              tasks[taskIdIndex].attachments.concat(
                linkAttachments[eachTaskId]
              );
            }
          });
        }
      ...
      attachmentOrTask =
        type === CONSTANTS.common.PROJECT_ATTACHMENT ? attachments : tasks;
...
function _projectTask(tasks, isImportedFromLibrary = false, parentTaskId = "") {
  tasks.forEach((singleTask) => {
    ...
    singleTask.type = singleTask.type
      ? singleTask.type
      : CONSTANTS.common.SIMPLE_TASK_TYPE;
    singleTask.status = singleTask.status
      ? singleTask.status
      : CONSTANTS.common.NOT_STARTED_STATUS;
    ...
    singleTask.createdAt = singleTask.createdAt
      ? singleTask.createdAt
      : new Date();
    singleTask.updatedAt = new Date();
    singleTask._id = UTILS.isValidMongoId(singleTask._id.toString())
      ? uuidv4()
      : singleTask._id;
      ...
      if (assessmentData.solutionDetails.isReusable) {
        let createdAssessment =
          await surveyService.createAssessmentSolutionFromTemplate(
            ...
            {
              name:
                assessmentData.solutionDetails.name + "-" + UTILS.epochTime(),
              description:
                assessmentData.solutionDetails.name + "-" + UTILS.epochTime(),
              program: {
        ...
        if (!createdAssessment.success) {
          throw {
            status: HTTP_STATUS_CODE["bad_request"].status,
            message:
              CONSTANTS.apiResponses.COULD_NOT_CREATE_ASSESSMENT_SOLUTION,
          };
        }
        ...
        if (!assignedAssessmentToUser.success) {
          throw {
            status: HTTP_STATUS_CODE["bad_request"].status,
            message:
              CONSTANTS.apiResponses.FAILED_TO_ASSIGNED_ASSESSMENT_TO_USER,
          };
        }
      ...
      if (observationData.solutionDetails.isReusable) {
        let observationCreatedFromTemplate =
          await surveyService.createObservationFromSolutionTemplate(
            ...
            {
              name:
                observationData.solutionDetails.name + "-" + UTILS.epochTime(),
              description:
                observationData.solutionDetails.name + "-" + UTILS.epochTime(),
              program: {
        ...
        result["observationId"] =
          observationCreatedFromTemplate.data.observationId;
      } else {
...
function _entitiesMetaInformation(entitiesData) {
  return new Promise(async (resolve, reject) => {
    ...
    for (let index = 0; index < entitiesData.length; index++) {
      let entityHierarchy = await userProfileService.getParentEntities(
        entitiesData[index]._id
      );
      ...
      entitiesData[index].metaInformation.entityType =
        entitiesData[index].entityType;
      entitiesData[index].metaInformation.registryDetails =
        entitiesData[index].registryDetails;
